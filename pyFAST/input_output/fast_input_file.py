...
class FASTInputFile(File):
    ...
    def __init__(self, filename=None, **kwargs):
        ...
        self.basefile = FASTInputFileBase(
            filename, **kwargs)  # Generic fileformat
...
class FASTInputFileBase(File):
    ...
    def keys(self):
        self.labels = [d['label'] for i, d in enumerate(self.data) if (
            not d['isComment']) and (i not in self._IComment)]
        return self.labels

    def getID(self, label):
        i = self.getIDSafe(label)
        if i < 0:
            raise KeyError('Variable `' + label +
                           '` not found in FAST file:'+self.filename)
        else:
            return i
        ...
        for i in I:
            if self.data[i]['tabType'] != TABTYPE_NOT_A_TAB:
                ...
                if 'tabDimVar' in self.data[i].keys():
                    ...
                    # Avoiding a recursive call to __setitem__ here
                    self.data[iDimVar]['value'] = nRows
                else:
                    pass
    ...
    def _read(self):
        ...
        NUMTAB_FROM_VAL_DETECT = ['HtFract', 'TwrElev', 'BlFract', 'Genspd_TLU', 'BlSpn', 'HvCoefID', 'AxCoefID',
                                  'JointID', 'Dpth', 'FillNumM', 'MGDpth', 'SimplCd', 'RNodes', 'kp_xr', 'mu1', 'TwrHtFr', 'TwrRe', 'WT_X']
        NUMTAB_FROM_VAL_DIM_VAR = ['NTwInpSt', 'NumTwrNds', 'NBlInpSt', 'DLL_NumTrq', 'NumBlNds', 'NHvCoef', 'NAxCoef',
                                   'NJoints', 'NCoefDpth', 'NFillGroups', 'NMGDepths', 1, 'BldNodes', 'kp_total', 1, 'NTwrHt', 'NTwrRe', 'NumTurbines']
        NUMTAB_FROM_VAL_VARNAME = ['TowProp', 'TowProp', 'BldProp', 'DLLProp', 'BldAeroNodes', 'HvCoefs', 'AxCoefs', 'Joints', 'DpthProp',
                                   'FillGroups', 'MGProp', 'SmplProp', 'BldAeroNodes', 'MemberGeom', 'DampingCoeffs', 'TowerProp', 'TowerRe', 'WindTurbines']
        NUMTAB_FROM_VAL_NHEADER = [2, 2, 2, 2, 2,
                                   2, 2, 2, 2, 2, 2, 2, 1, 2, 2, 1, 1, 2]
        NUMTAB_FROM_VAL_TYPE = ['num', 'num', 'num', 'num', 'num', 'num', 'num',
                                'num', 'num', 'num', 'num', 'num', 'mix', 'num', 'num', 'num', 'num', 'mix']
        # SubDyn
        NUMTAB_FROM_VAL_DETECT += ['RJointID',
                                   'IJointID', 'COSMID', 'CMJointID']
        NUMTAB_FROM_VAL_DIM_VAR += ['NReact', 'NInterf', 'NCOSMs', 'NCmass']
        NUMTAB_FROM_VAL_VARNAME += ['BaseJoints', 'InterfaceJoints',
                                    'MemberCosineMatrix', 'ConcentratedMasses']
        ...
        NUMTAB_FROM_LAB_DETECT = [
            'NumAlf', 'F_X', 'MemberCd1', 'MJointID1', 'NOutLoc', 'NOutCnt', 'PropD']
        NUMTAB_FROM_LAB_DIM_VAR = [
            'NumAlf', 'NKInpSt', 'NCoefMembers', 'NMembers', 'NMOutputs', 'NMOutputs', 'NPropSets']
        NUMTAB_FROM_LAB_VARNAME = ['AFCoeff', 'TMDspProp', 'MemberProp',
                                   'Members', 'MemberOuts', 'MemberOuts', 'SectionProp']
        NUMTAB_FROM_LAB_NHEADER = [2, 2, 2, 2, 2, 2, 2]
        NUMTAB_FROM_LAB_NOFFSET = [0, 0, 0, 0, 0, 0, 0]
        NUMTAB_FROM_LAB_TYPE = ['num', 'num',
                                'num', 'mix', 'num', 'sdout', 'num']
        # MoorDyn Version 1 and 2 (with AUTO for LAB_DIM_VAR)
        NUMTAB_FROM_LAB_DETECT += ['Diam', 'Type', 'LineType', 'Attachment']
        NUMTAB_FROM_LAB_DIM_VAR += ['NTypes:AUTO',
                                    'NConnects', 'NLines:AUTO', 'AUTO']
        NUMTAB_FROM_LAB_VARNAME += ['LineTypes',
                                    'ConnectionProp', 'LineProp', 'Points']
        NUMTAB_FROM_LAB_NHEADER += [2, 2, 2, 2]
        NUMTAB_FROM_LAB_NOFFSET += [0, 0, 0, 0]
        NUMTAB_FROM_LAB_TYPE += ['mix', 'mix', 'mix', 'mix']
        # SubDyn
        NUMTAB_FROM_LAB_DETECT += ['GuyanDampSize',
                                   'YoungE', 'YoungE', 'EA', 'MatDens']
        NUMTAB_FROM_LAB_DIM_VAR += [6, 'NPropSets',
                                    'NXPropSets', 'NCablePropSets', 'NRigidPropSets']
        NUMTAB_FROM_LAB_VARNAME += ['GuyanDampMatrix',
                                    'BeamProp', 'BeamPropX', 'CableProp', 'RigidProp']
        ...
        while i < len(lines):
            ...
            if line.upper().find('ADDITIONAL OUTPUTS') > 0 \
                ...
                if i+2 < len(lines) and (lines[i+2].lower().find('bldnd_bladesout') > 0 or lines[i+2].lower().find('bldnd_bloutnd') > 0):
                    ...
                    self.data.append(parseFASTInputLine(
                        'END of input file (the word "END" must appear in the first 3 columns of this last OutList line)', i+1))
                    self.data.append(parseFASTInputLine(
                        '---------------------------------------------------------------------------------------', i+2))
                    break
            ...
            if labelRaw == 'kp_total':
                ...
                if isStr(d['value']):
                    ...
                    if int(d['value']) <= 0:
                        ...
                        i += 1
                        # 3 comment lines
                        self.data.append(parseFASTInputLine(lines[i], i))
                        i += 1
                        self.data.append(parseFASTInputLine(lines[i], i))
                        i += 1
                        self.data.append(parseFASTInputLine(lines[i], i))
                        i += 1
                        splits = cleanAfterChar(
                        ...
                        try:
                            ...
                            raise WrongFormatError(
                                'Wrong format while reading coordinates of airfoil reference')
                        i += 1
                        d = getDict()
                        ...
                        nTabLines = self[d['tabDimVar']] - \
                            1  # SOMEHOW ONE DATA POINT LESS
                        d['value'], d['tabColumnNames'], _ = parseFASTNumTable(
                            self.filename, lines[i:i+nTabLines+1], nTabLines, i, 1)
                        d['tabUnits'] = ['(-)', '(-)']
                        self.data.append(d)
                        break
            ...
            if isStr(d['value']) and d['value'].lower() in NUMTAB_FROM_VAL_DETECT_L:
                ...
                d['value'], d['tabColumnNames'], d['tabUnits'] = parseFASTNumTable(
                    self.filename, lines[i:i+nTabLines+nHeaders], nTabLines, i, nHeaders, tableType=tab_type, varNumLines=d['tabDimVar'])
                ...
                if labelRaw == 'numalf':
                    ...
                    d = {'value': None, 'label': 'NumAlf'+labOffset,
                         'isComment': False, 'descr': '', 'tabType': None}
                    i += 1
                nHeaders = NUMTAB_FROM_LAB_NHEADER[ii]
                nOffset = NUMTAB_FROM_LAB_NOFFSET[ii]
                if nOffset > 0:
                    # Creating a dictionary for that entry
                    dd = {'value': d['value'], 'label': d['label']+labOffset,
                          'isComment': False, 'descr': d['descr'], 'tabType': TABTYPE_NOT_A_TAB}
                    self.data.append(dd)
                ...
                if isinstance(tabDimVar, int):  # dimension hardcoded
                    ...
                    for tabDimVar in tabDimVars:
                        ...
                        if tabDimVar == 'AUTO':
                            ...
                            nTabLines = findNumberOfTableLines(
                                lines[i+nHeaders:], break_chars=['---', '!', '#'])
                            break
                        else:
                            try:
                ...
                #print('Reading table {} Dimension {} (based on {})'.format(d['label'],nTabLines,d['tabDimVar']));
                d['value'], d['tabColumnNames'], d['tabUnits'] = parseFASTNumTable(
                    self.filename, lines[i:i+nTabLines+nHeaders+nOffset], nTabLines, i, nHeaders, tableType=tab_type, nOffset=nOffset, varNumLines=d['tabDimVar'])
                ...
                nTabLines = self[d['tabDimVar']]
                #print('Reading table {} Dimension {} (based on {})'.format(d['label'],nTabLines,d['tabDimVar']));
                d['value'] = parseFASTFilTable(
                    lines[i:i+nTabLines], nTabLines, i)
                i += nTabLines-1
            ...
            if d['isComment']:
                ...
                if hasSpecialChars(d['label']):
                    ...
                    if i > 3:  # first few lines may be comments, we allow it
                        ...
                        raise WrongFormatError(
                            'Special Character found in Label: `{}`, for line: `{}`'.format(d['label'], line))
                if len(d['label']) == 0:
                    nWrongLabels += 1
            if nComments > len(lines)*0.35:
                #print('Comment fail',nComments,len(lines),self.filename)
                raise WrongFormatError(
                    'Most lines were read as comments, probably not a FAST Input File: {}'.format(self.filename))
            if nWrongLabels > len(lines)*0.10:
                #print('Label fail',nWrongLabels,len(lines),self.filename)
                raise WrongFormatError(
                    'Too many lines with wrong labels, probably not a FAST Input File {}:'.format(self.filename))

            # --- END OF FOR LOOP ON LINES

        # --- PostReading checks
        labels = self.keys()
        duplicates = set([x for x in labels if (labels.count(
            x) > 1) and x != 'OutList' and x.strip() != '-'])
        if len(duplicates) > 0:
        ...
        for i in range(len(self.data)):
            ...
            if d['isComment']:
                ...
                if d['tabColumnNames'] is not None:
                    s += '{}'.format(' '.join(['{:15s}'.format(s)
                                     for s in d['tabColumnNames']]))
                ...
                if np.size(d['value'], 0) > 0:
                    s += newline
                    s += '\n'.join('\t'.join(('{:15.0f}'.format(x) if int(
                        x) == x else '{:15.8e}'.format(x)) for x in y) for y in d['value'])
                ...
                if np.size(d['value'], 0) > 0:
                    s += '\n'
                    s += '\n'.join('\t'.join(toStringIntFloatStr(x)
                                   for x in y) for y in d['value'])
            elif d['tabType'] == TABTYPE_NUM_WITH_HEADERCOM:
                s += '! {}\n'.format(' '.join(['{:15s}'.format(s)
                                     for s in d['tabColumnNames']]))
                s += '! {}\n'.format(' '.join(['{:15s}'.format(s)
                                     for s in d['tabUnits']]))
                s += '\n'.join('\t'.join('{:15.8e}'.format(x)
                               for x in y) for y in d['value'])
        ...
        for i in range(len(self.data)):
            ...
            if d['tabType'] in [TABTYPE_NUM_WITH_HEADER, TABTYPE_NUM_WITH_HEADERCOM, TABTYPE_NUM_NO_HEADER, TABTYPE_MIX_WITH_HEADER]:
                ...
                if d['tabUnits'] is None:
                    ...
                    Cols = ['{}_{}'.format(c, u.replace('(', '[').replace(')', ']')) for c, u in zip(
                        d['tabColumnNames'], d['tabUnits'])]
                # print(Val)
                # print(Cols)
                ...
                if self.getIDSafe('TwFAM1Sh(2)') > 0:
                    ...
                    Cols = Cols + ['ShapeForeAft1_[-]', 'ShapeForeAft2_[-]',
                                   'ShapeSideSide1_[-]', 'ShapeSideSide2_[-]']
                ...
                IAll = range(1+36+36)
                IMain = [0] + [i*6+i +
                               1 for i in range(6)] + [i*6+i+37 for i in range(6)]
    ...
    def detectAndReadAirfoilAD14(self, lines):
        ...
        if len(KW1) > nTables and len(KW2) > nTables:
            if KW1[nTables].lower() == 'angle' and KW2[nTables].lower() == 'minimum':
                ...
                data = np.array([lines[i].strip().split()
                                for i in range(14, nTabLines+14)]).astype(float)
                # print(data)
                d = getDict()
                d['label'] = 'Polar'
                ...
                if np.size(data, 1) == 1+nTables*3:
                    d['tabColumnNames'] = ['Alpha'] + \
                        [n+l for l in TableID for n in ['Cl', 'Cd', 'Cm']]
                    d['tabUnits'] = ['(deg)']+['(-)', '(-)', '(-)']*nTables
                elif np.size(data, 1) == 1+nTables*2:
                    d['tabColumnNames'] = ['Alpha'] + \
                        [n+l for l in TableID for n in ['Cl', 'Cd']]
                    d['tabUnits'] = ['(deg)']+['(-)', '(-)']*nTables
                else:
                    d['tabColumnNames'] = ['col{}'.format(
                        j) for j in range(np.size(data, 1))]
                self.data.append(d)
                return True
    ...
    def readBeamDynProps(self, lines, iStart):
        ...
        try:
            for j in range(nStations):
                ...
                K[j, :, :] = np.array(
                    (' '.join(lines[i:i+6])).split()).astype(float).reshape(6, 6)
                i += 7
                # Read mass matrix
                M[j, :, :] = np.array(
                    (' '.join(lines[i:i+6])).split()).astype(float).reshape(6, 6)
            ...
            raise WrongFormatError(
                'An error occured while reading section {}/{}'.format(j+1, nStations))
        d = getDict()
    ...
    try:
        if nHeaders == 0:
            ...
            if tableType == 'num':
                ...
                raise NotImplementedError(
                    'Reading FAST tables with no headers for type different than num not implemented yet')
        ...
        if nHeaders >= 2:
            ...
            if len(ColNames) != len(Units):
                print(ColNames)
                print(Units)
                print('[WARN] {}: Line {}: Number of column names different from number of units in table'.format(
                    filename, iStart+i+1))
            ...
            for i in range(nHeaders+nOffset, n+nHeaders+nOffset):
                ...
                if len(v) != nCols:
                    ...
                    if ColNames[-1].lower() not in ['nodecnt']:
                        print('[WARN] {}: Line {}: number of data different from number of column names. ColumnNames: {}'.format(
                            filename, iStart+i+1, ColNames))
                ...
                v = [s.replace('true', '1').replace('false', '0').replace(
                    'noprint', '0').replace('print', '1') for s in v]
                v = [float(s) for s in v[0:nCols]]
                if len(v) < nCols:
                    raise Exception(
                        'Number of data is lower than number of column names')
            ...
            for i in range(nHeaders+nOffset, n+nHeaders+nOffset):
                ...
                if l.startswith('---'):
                    raise BrokenFormatError(
                        'Error reading line {} while reading table. Is the variable `{}` set correctly?'.format(iStart+i+1, varNumLines))
                if len(v) != nCols:
                    # Discarding SubDyn special cases
                    if ColNames[-1].lower() not in ['cosmid', 'ssifile']:
                        print('[WARN] {}: Line {}: Number of data is different than number of column names. Column Names: {}'.format(
                            filename, iStart+1+i, ColNames))
                if i == nHeaders+nOffset:
                    if len(v) > nCols:
                        ColNames = ColNames+['Col']*(len(v)-nCols)
                        Units = Units+['Col']*(len(v)-nCols)
...
def parseFASTFilTable(lines, n, iStart):
    ...
    try:
        ...
        if len(lines) != n:
            raise WrongFormatError(
                'Not enough lines in table: {} lines instead of {}'.format(len(lines), n))
        for i in range(n):
            l = lines[i].split()
            # print(l[0].strip())
...
class BDFile(FASTInputFileBase):
    ...
    def from_fast_input_file(cls, parent):
        self = cls()
        self.setData(filename=parent.filename, data=parent.data,
                     hasNodal=parent.hasNodal, module='BD')
        return self
    ...
    def __init__(self, filename=None, **kwargs):
        ...
        if filename is None:
            ...
            self.addValKey(True, 'QuasiStaticInit',
                           'Use quasi-static pre-conditioning with centripetal accelerations in initialization? (flag) [dynamic solve only]')
            self.addValKey(
            ...
            self.addValKey("DEFAULT", 'load_retries',
                           'Number of factored load retries before quitting the simulation [DEFAULT = 20]')
            self.addValKey(
            ...
            self.addValKey("DEFAULT", 'tngt_stf_fd',
                           'Use finite differenced tangent stiffness matrix? (flag)')
            self.addValKey("DEFAULT", 'tngt_stf_comp',
                           'Compare analytical finite differenced tangent stiffness matrix? (flag)')
            self.addValKey("DEFAULT", 'tngt_stf_pert',
                           'Perturbation size for finite differencing (-) [DEFAULT = 1E-6]')
            self.addValKey("DEFAULT", 'tngt_stf_difftol',
                           'Maximum allowable relative difference between analytical and fd tangent stiffness (-); [DEFAULT = 0.1]')
            self.addValKey(
            ...
            self.addValKey(
                0, 'kp_total', 'Total number of key points (-) [must be at least 3]')
            self.addValKey([1, 0], 'kp_per_member',
            ...
            self.addComment(
                '---------------------- MESH PARAMETER ------------------------------------------')
            self.addValKey(5, 'order_elem',
                           'Order of interpolation (basis) function (-)')
            self.addComment(
                '---------------------- MATERIAL PARAMETER --------------------------------------')
            self.addValKey('"undefined"', 'BldFile',
                           'Name of file containing properties for blade (quoted string)')
            self.addComment(
                '---------------------- PITCH ACTUATOR PARAMETERS -------------------------------')
            self.addValKey(False, 'UsePitchAct',
                           'Whether a pitch actuator should be used (flag)')
            self.addValKey(
                1, 'PitchJ', 'Pitch actuator inertia (kg-m^2) [used only when UsePitchAct is true]')
            self.addValKey(
            ...
            self.addComment(
                '---------------------- OUTPUTS -------------------------------------------------')
            self.addValKey(False, 'SumPrint',
                           'Print summary data to "<RootName>.sum" (flag)')
            self.addValKey('"ES10.3E2"', 'OutFmt',
            ...
            self.addValKey(
                [''], 'OutList', 'The next line(s) contains a list of output parameters. See OutListParameters.xlsx, BeamDyn tab for a listing of available output channels, (-)')
            self.addComment(
                'END of OutList (the word "END" must appear in the first 3 columns of this last OutList line)')
            self.addComment(
                '---------------------- NODE OUTPUTS --------------------------------------------')
            self.addValKey(99, 'BldNd_BlOutNd',
                           'Blade nodes on each blade (currently unused)')
            self.addValKey(
                [''], 'OutList_Nodal', 'The next line(s) contains a list of output parameters.  See OutListParameters.xlsx, BeamDyn_Nodes tab for a listing of available output channels, (-)')
            self.addComment(
            ...
            self.data[1] = {'value': self._lines[1], 'label': '',
                            'isComment': True, 'descr': '', 'tabType': 0}
    ...
    def _writeSanityChecks(self):
        ...
        self.data[i+1]['value'] = [1,
                                   self['MemberGeom'].shape[0]]  # kp_per_member
        self.data[i+1]['label'] = 'kp_per_member'
...
class EDBladeFile(FASTInputFileBase):
    ...
    def from_fast_input_file(cls, parent):
        self = cls()
        self.setData(filename=parent.filename, data=parent.data,
                     hasNodal=parent.hasNodal, module='EDBlade')
        return self
    ...
    def __init__(self, filename=None, **kwargs):
        ...
        if filename is None:
            ...
            self.addComment(
                'ElastoDyn blade definition, written by EDBladeFile.')
            self.addComment(
                '---------------------- BLADE PARAMETERS ----------------------------------------')
            ...
            self.addValKey(1., 'FlStTunr(1)',
                           'Blade flapwise modal stiffness tuner, 1st mode (-)')
            self.addValKey(1., 'FlStTunr(2)',
            ...
            self.addTable('BldProp', np.zeros((0, 6)), tabType=1, tabDimVar='NBlInpSt', cols=[
                          'BlFract', 'PitchAxis', 'StrcTwst', 'BMassDen', 'FlpStff', 'EdgStff'], units=['(-)', '(-)', '(deg)', '(kg/m)', '(Nm^2)', '(Nm^2)'])
            self.addComment(
                '---------------------- BLADE MODE SHAPES ---------------------------------------')
            ...
            # NOTE: using something not too bad just incase user uses these as is..
            self.addValKey(0.0, 'BldFl2Sh(2)', 'Flap mode 2, coeff of x^2')
    ...
    def _writeSanityChecks(self):
        ...
        for s in ['BldFl1Sh', 'BldFl2Sh', 'BldEdgSh']:
            sumcoeff = np.sum([self[s+'('+str(i)+')']
                              for i in [2, 3, 4, 5, 6]])
            if np.abs(sumcoeff-1) > 1e-4:
                print('[WARN] Sum of coefficients for polynomial {} not equal to 1 ({}). File: {}'.format(
                    s, sumcoeff, self.filename))
...
class EDTowerFile(FASTInputFileBase):
    ...
    def from_fast_input_file(cls, parent):
        self = cls()
        self.setData(filename=parent.filename, data=parent.data,
                     hasNodal=parent.hasNodal, module='EDTower')
        return self
    ...
    def __init__(self, filename=None, **kwargs):
        ...
        if filename is None:
            ...
            self.addComment(
                'ElastoDyn tower definition, written by EDTowerFile.')
            self.addComment(
                '---------------------- TOWER PARAMETERS ----------------------------------------')
            ...
            self.addValKey(1.0, 'TwFAM1Sh(2)',
                           'Mode 1, coefficient of x^2 term')
            self.addValKey(0.0, 'TwFAM1Sh(3)',
                           '      , coefficient of x^3 term')
            self.addValKey(0.0, 'TwFAM1Sh(4)',
                           '      , coefficient of x^4 term')
            self.addValKey(0.0, 'TwFAM1Sh(5)',
                           '      , coefficient of x^5 term')
            self.addValKey(0.0, 'TwFAM1Sh(6)',
                           '      , coefficient of x^6 term')
            # NOTE: using something not too bad just incase user uses these as is..
            self.addValKey(-26., 'TwFAM2Sh(2)',
                           'Mode 2, coefficient of x^2 term')
            self.addValKey(0.0, 'TwFAM2Sh(3)',
                           '      , coefficient of x^3 term')
            self.addValKey(27., 'TwFAM2Sh(4)',
                           '      , coefficient of x^4 term')
            self.addValKey(0.0, 'TwFAM2Sh(5)',
                           '      , coefficient of x^5 term')
            self.addValKey(0.0, 'TwFAM2Sh(6)',
                           '      , coefficient of x^6 term')
            self.addComment(
                '---------------------- TOWER SIDE-TO-SIDE MODE SHAPES --------------------------')
            self.addValKey(1.0, 'TwSSM1Sh(2)',
                           'Mode 1, coefficient of x^2 term')
            self.addValKey(0.0, 'TwSSM1Sh(3)',
                           '      , coefficient of x^3 term')
            self.addValKey(0.0, 'TwSSM1Sh(4)',
                           '      , coefficient of x^4 term')
            self.addValKey(0.0, 'TwSSM1Sh(5)',
                           '      , coefficient of x^5 term')
            self.addValKey(0.0, 'TwSSM1Sh(6)',
                           '      , coefficient of x^6 term')
            # NOTE: using something not too bad just incase user uses these as is..
            self.addValKey(-26., 'TwSSM2Sh(2)',
                           'Mode 2, coefficient of x^2 term')
            self.addValKey(0.0, 'TwSSM2Sh(3)',
                           '      , coefficient of x^3 term')
            self.addValKey(27., 'TwSSM2Sh(4)',
                           '      , coefficient of x^4 term')
            self.addValKey(0.0, 'TwSSM2Sh(5)',
                           '      , coefficient of x^5 term')
            self.addValKey(0.0, 'TwSSM2Sh(6)',
                           '      , coefficient of x^6 term')
        else:
            # fix some stuff that generic reader fail at
            self.data[1] = {'value': self._lines[1], 'label': '',
    ...
    def _toDataFrame(self):
        ...
        Modes[:, 3] = x**2 * self['TwSSM2Sh(2)'] \
            ...
            + x**5 * self['TwSSM2Sh(5)'] \
            + x**6 * self['TwSSM2Sh(6)']
        df[['ShapeForeAft1_[-]', 'ShapeForeAft2_[-]',
            'ShapeSideSide1_[-]', 'ShapeSideSide2_[-]']] = Modes
        return df
...
class ADBladeFile(FASTInputFileBase):
    ...
    def from_fast_input_file(cls, parent):
        self = cls()
        self.setData(filename=parent.filename, data=parent.data,
                     hasNodal=parent.hasNodal, module='ADBlade')
        return self
    ...
    def __init__(self, filename=None, **kwargs):
        ...
        if filename is None:
            # Define a prototype for this file format
            self.addComment(
                '------- AERODYN BLADE DEFINITION INPUT FILE ----------------------------------------------')
            self.addComment(
                'Aerodynamic blade definition, written by ADBladeFile')
            self.addComment(
                '======  Blade Properties =================================================================')
            self.addKeyVal(
                'NumBlNds', 0, 'Number of blade nodes used in the analysis (-)')
            self.addTable('BldAeroNodes', np.zeros((0, 7)), tabType=1, tabDimVar='NumBlNds', cols=[
                          'BlSpn', 'BlCrvAC', 'BlSwpAC', 'BlCrvAng', 'BlTwist', 'BlChord', 'BlAFID'], units=['(m)', '(m)', '(m)', '(deg)', '(deg)', '(m)', '(-)'])
        self.module = 'ADBlade'
    ...
    def _toDataFrame(self):
        ...
        # --- Adding C2 axis
        # distance (in chord) from leading edge to aero center
        ACloc = r*0 + 0.25
...
class ADPolarFile(FASTInputFileBase):
    ...
    def from_fast_input_file(cls, parent):
        self = cls()
        self.setData(filename=parent.filename, data=parent.data,
                     hasNodal=parent.hasNodal, module='ADPolar')
        return self
    ...
    def __init__(self, filename=None, hasUA=True, numTabs=1, **kwargs):
        ...
        if filename is None:
            ...
            self.addComment(
                '! ------------------------------------------------------------------------------')
            self.addValKey("DEFAULT", 'InterpOrd',
                           'Interpolation order to use for quasi-steady table lookup {1=linear; 3=cubic spline; "default"} [default=3]')
            self.addValKey(
                1, 'NonDimArea', 'The non-dimensional area of the airfoil (area/chord^2) (set to 1.0 if unsure or unneeded)')
            self.addValKey(
                0, 'NumCoords', 'The number of coordinates in the airfoil shape file.  Set to zero if coordinates not included.')
            self.addValKey(
                numTabs, 'NumTabs', 'Number of airfoil tables in this file.  Each table must have lines for Re and Ctrl.')
            ...
            for iTab in range(numTabs):
                ...
                self.addComment(
                    '! ------------------------------------------------------------------------------')
                self.addValKey(1.0,    'Re' + labOffset,
                ...
                if hasUA:
                    ...
                    self.addComment(
                        '!........................................')
                    self.addValKey(np.nan, 'alpha0' + labOffset,
                                   r"0-lift angle of attack, depends on airfoil.")
                    self.addValKey(np.nan, 'alpha1' + labOffset,
                                   r"Angle of attack at f=0.7, (approximately the stall angle) for AOA>alpha0. (deg)")
                    self.addValKey(np.nan, 'alpha2' + labOffset,
                                   r"Angle of attack at f=0.7, (approximately the stall angle) for AOA<alpha0. (deg)")
                    self.addValKey(
                        1, 'eta_e' + labOffset, r"Recovery factor in the range [0.85 - 0.95] used only for UAMOD=1, it is set to 1 in the code when flookup=True. (-)")
                    self.addValKey(np.nan, 'C_nalpha' + labOffset,
                                   r"Slope of the 2D normal force coefficient curve. (1/rad)")
                    self.addValKey("DEFAULT", 'T_f0' + labOffset,
                                   r"Initial value of the time constant associated with Df in the expression of Df and f''. [default = 3]")
                    self.addValKey("DEFAULT", 'T_V0' + labOffset,
                                   r"Initial value of the time constant associated with the vortex lift decay process; it is used in the expression of Cvn. It depends on Re,M, and airfoil class. [default = 6]")
                    self.addValKey("DEFAULT", 'T_p' + labOffset,
                                   r"Boundary-layer,leading edge pressure gradient time constant in the expression of Dp. It should be tuned based on airfoil experimental data. [default = 1.7]")
                    self.addValKey("DEFAULT", 'T_VL' + labOffset,
                                   r"Initial value of the time constant associated with the vortex advection process; it represents the non-dimensional time in semi-chords, needed for a vortex to travel from LE to trailing edge (TE); it is used in the expression of Cvn. It depends on Re, M (weakly), and airfoil. [valid range = 6 - 13, default = 11]")
                    self.addValKey("DEFAULT", 'b1' + labOffset,
                                   r"Constant in the expression of phi_alpha^c and phi_q^c.  This value is relatively insensitive for thin airfoils, but may be different for turbine airfoils. [from experimental results, defaults to 0.14]")
                    self.addValKey("DEFAULT", 'b2' + labOffset,
                                   r"Constant in the expression of phi_alpha^c and phi_q^c.  This value is relatively insensitive for thin airfoils, but may be different for turbine airfoils. [from experimental results, defaults to 0.53]")
                    self.addValKey("DEFAULT", 'b5' + labOffset,
                                   r"Constant in the expression of K'''_q,Cm_q^nc, and k_m,q.  [from  experimental results, defaults to 5]")
                    self.addValKey("DEFAULT", 'A1' + labOffset,
                                   r"Constant in the expression of phi_alpha^c and phi_q^c.  This value is relatively insensitive for thin airfoils, but may be different for turbine airfoils. [from experimental results, defaults to 0.3]")
                    self.addValKey("DEFAULT", 'A2' + labOffset,
                                   r"Constant in the expression of phi_alpha^c and phi_q^c.  This value is relatively insensitive for thin airfoils, but may be different for turbine airfoils. [from experimental results, defaults to 0.7]")
                    self.addValKey("DEFAULT", 'A5' + labOffset,
                                   r"Constant in the expression of K'''_q,Cm_q^nc, and k_m,q. [from experimental results, defaults to 1]")
                    self.addValKey(
                        0, 'S1' + labOffset, r"Constant in the f curve best-fit for alpha0<=AOA<=alpha1; by definition it depends on the airfoil. [ignored if UAMod<>1]")
                    self.addValKey(
                        0, 'S2' + labOffset, r"Constant in the f curve best-fit for         AOA> alpha1; by definition it depends on the airfoil. [ignored if UAMod<>1]")
                    self.addValKey(
                        0, 'S3' + labOffset, r"Constant in the f curve best-fit for alpha2<=AOA< alpha0; by definition it depends on the airfoil. [ignored if UAMod<>1]")
                    self.addValKey(
                        0, 'S4' + labOffset, r"Constant in the f curve best-fit for         AOA< alpha2; by definition it depends on the airfoil. [ignored if UAMod<>1]")
                    ...
                    self.addValKey(np.nan, 'Cn2' + labOffset,
                                   r"As Cn1 for negative AOAs.")
                    self.addValKey("DEFAULT", 'St_sh' + labOffset,
                                   r"Strouhal's shedding frequency constant.  [default = 0.19]")
                    self.addValKey(np.nan, 'Cd0' + labOffset,
                                   r"2D drag coefficient value at 0-lift.")
                    self.addValKey(
                        np.nan, 'Cm0' + labOffset, r"2D pitching moment coefficient about 1/4-chord location, at 0-lift, positive if nose up. [If the aerodynamics coefficients table does not include a column for Cm, this needs to be set to 0.0]")
                    self.addValKey(
                        0, 'k0' + labOffset, r"Constant in the \hat(x)_cp curve best-fit; = (\hat(x)_AC-0.25).  [ignored if UAMod<>1]")
                    self.addValKey(
                        0, 'k1' + labOffset, r"Constant in the \hat(x)_cp curve best-fit.  [ignored if UAMod<>1]")
                    self.addValKey(
                        0, 'k2' + labOffset, r"Constant in the \hat(x)_cp curve best-fit.  [ignored if UAMod<>1]")
                    self.addValKey(
                        0, 'k3' + labOffset, r"Constant in the \hat(x)_cp curve best-fit.  [ignored if UAMod<>1]")
                    self.addValKey(
                        0, 'k1_hat' + labOffset, r"Constant in the expression of Cc due to leading edge vortex effects.  [ignored if UAMod<>1]")
                    self.addValKey("DEFAULT", 'x_cp_bar' + labOffset,
                                   r"Constant in the expression of \hat(x)_cp^v. [ignored if UAMod<>1, default = 0.2]")
                    self.addValKey("DEFAULT", 'UACutout' + labOffset,
                                   r"Angle of attack above which unsteady aerodynamics are disabled (deg). [Specifying the string 'Default' sets UACutout to 45 degrees]")
                    self.addValKey("DEFAULT", 'filtCutOff' + labOffset,
                                   r"Reduced frequency cut-off for low-pass filtering the AoA input to UA, as well as the 1st and 2nd derivatives (-) [default = 0.5]")
                    self.addComment(
                        '!........................................')
                else:
                    self.addValKey(False,    'InclUAdata'+labOffset,
                                   'Is unsteady aerodynamics data included in this table? If TRUE, then include 30 UA coefficients below this line')
                self.addComment('! Table of aerodynamics coefficients')
                self.addValKey(0,    'NumAlf'+labOffset,
                               '! Number of data lines in the following table')
                self.addTable('AFCoeff'+labOffset, np.zeros((0, 4)), tabType=2, tabDimVar='NumAlf'+labOffset,
                              cols=['Alpha', 'Cl', 'Cd', 'Cm'], units=['(deg)', '(-)', '(-)', '(-)'])
        self.module = 'ADPolar'
    ...
    def _write(self):
        ...
        if nTabs == 1:
            ...
            self._writeSanityChecks()
            Labs = ['Re', 'Ctrl', 'UserProp', 'alpha0', 'alpha1', 'alpha2', 'eta_e', 'C_nalpha', 'T_f0', 'T_V0', 'T_p', 'T_VL', 'b1', 'b2', 'b5', 'A1', 'A2', 'A5', 'S1',
                    'S2', 'S3', 'S4', 'Cn1', 'Cn2', 'St_sh', 'Cd0', 'Cm0', 'k0', 'k1', 'k2', 'k3', 'k1_hat', 'x_cp_bar', 'UACutout', 'filtCutOff', 'InclUAdata', 'NumAlf', 'AFCoeff']
            # Store all labels
    ...
    def _toDataFrame(self):
        ...
        for k, df in dfs.items():
            ...
            # Linear Cn
            try:
                CnLin_ = self['C_nalpha'+labOffset] * \
                    (alpha-self['alpha0'+labOffset]*np.pi/180.)
                CnLin = CnLin_.copy()
            ...
            try:
                iBef2 = np.where(
                    alpha < self['alpha2'+labOffset]*np.pi/180.)[0][-1]
                iBef1 = np.where(
                    alpha < self['alpha1'+labOffset]*np.pi/180.)[0][-1]
                iBef0 = np.where(
                    alpha < self['alpha0'+labOffset]*np.pi/180.)[0][-1]
        ...
        while i < len(lines):
            ...
            if l.find('!mass') == 0:
                ...
                if nDOF < -1 or nDOF != nDOFCommon:
                    raise BrokenFormatError(
                        'ExtPtfm stiffness matrix nDOF issue. nDOF common: {}, nDOF provided: {}'.format(nDOFCommon, nDOF))
                ...
                if nDOF < -1 or nDOF != nDOFCommon:
                    raise BrokenFormatError(
                        'ExtPtfm stiffness matrix nDOF issue nDOF common: {}, nDOF provided: {}'.format(nDOFCommon, nDOF))
                self.addKeyVal('StiffnessMatrix',
                               readmat(nDOF, nDOF, lines, i+2))
                i = i+1+nDOF
                ...
                if nDOF < -1 or nDOF != nDOFCommon:
                    raise BrokenFormatError(
                        'ExtPtfm damping matrix nDOF issue nDOF common: {}, nDOF provided: {}'.format(nDOFCommon, nDOF))
                self.addKeyVal('DampingMatrix', readmat(
                    nDOF, nDOF, lines, i+2))
                i = i+1+nDOF
                ...
                try:
                    ...
                    raise BrokenFormatError(
                        'Cannot read loading since time step and simulation time not properly set.')
                self.addKeyVal('Loading', readmat(
                    nt, nDOFCommon+1, lines, i+2))
                i = i+nt+1
            elif len(l) > 0:
                if l[0] == '!':
                    ...
                    raise BrokenFormatError(
                        'Unexcepted content found on line {}'.format(i))
            i += 1
...
class ExtPtfmFile(FASTInputFileBase):
    ...
    def from_fast_input_file(cls, parent):
        self = cls()
        self.setData(filename=parent.filename, data=parent.data,
                     hasNodal=parent.hasNodal, module='ExtPtfm')
        return self
    ...
    def toString(self):
        ...
        s += '\n!Mass Matrix\n'
        s += '!Dimension: {}\n'.format(self['nDOF'])
        s += '\n'.join(''.join('{:16.8e}'.format(x)
                       for x in y) for y in self['MassMatrix'])

        s += '\n\n!Stiffness Matrix\n'
        s += '!Dimension: {}\n'.format(self['nDOF'])
        s += '\n'.join(''.join('{:16.8e}'.format(x)
                       for x in y) for y in self['StiffnessMatrix'])

        s += '\n\n!Damping Matrix\n'
        s += '!Dimension: {}\n'.format(self['nDOF'])
        s += '\n'.join(''.join('{:16.8e}'.format(x)
                       for x in y) for y in self['DampingMatrix'])

        s += '\n\n!Loading and Wave Elevation\n'
        s += '!Dimension: 1 time column -  {} force columns\n'.format(
            self['nDOF'])
        s += '\n'.join(''.join('{:16.8e}'.format(x)
                       for x in y) for y in self['Loading'])
        return s
    ...
    def _toDataFrame(self):
        ...
        nDOF = self['nDOF']
        Cols = ['Time_[s]', 'InpF_Fx_[N]', 'InpF_Fy_[N]',
                'InpF_Fz_[N]', 'InpF_Mx_[Nm]', 'InpF_My_[Nm]', 'InpF_Mz_[Nm]']
