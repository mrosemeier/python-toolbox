...
class Polar(object):
    ...
    def __init__(self, filename=None, alpha=None, cl=None, cd=None, cm=None, Re=None,
        ...
        if filename is not None:
            df, Re = loadPolarFile(
                filename, fformat=fformat, to_radians=radians, verbose=verbose)
            ...
            if nLocks > 0 and nLocks < 3:
                raise Exception(
                    "For now, input files are assumed to have all or none of the columns: (fs, cl_fs, and cl_inv). Otherwise, we\'ll have to ensure consitency, and so far we dont...")
    ...
    def interpolant(self, variables=['cl', 'cd', 'cm'], radians=None):
        ...
        for v in variables:
            ...
            if v not in MAP.keys():
                raise Exception(
                    'Polar: cannot create an interpolant for variable `{}`, allowed variables: {}'.format(v, MAP.keys()))
    ...
    def cl_lin(self):  # TODO consider removing
        print(
            '[WARN] Polar: cl_lin is a bit of a weird property. Not sure if it will be kept')
    ...
    def fromfile(cls, filename, fformat='auto', compute_params=False, to_radians=False):
        ...
        """
        print(
            '[WARN] Polar: "fromfile" is depreciated and will be removed in a future release')
    ...
    def correction3D(
        ...
        if lift_method == "DuSelig":
            ...
            raise Exception(
                "The keyword argument lift_method (3d correction for lift) can only be DuSelig or Snel.")
        ...
        if blending_method == "linear_25_45":
            ...
            delta_corr = 10
            y1 = 1.0 - \
                smooth_heaviside(alpha, k=1, rng=(
                    alpha_max_corr, alpha_max_corr + np.deg2rad(delta_corr)))
            y2 = smooth_heaviside(alpha, k=1, rng=(
                0.0, np.deg2rad(delta_corr)))
            adj = y1 * y2
        else:
        ...
        if drag_method == "Eggers":
            delta_cd = cl_corr * (np.sin(alpha) - 0.12 * np.cos(alpha)) / \
                (np.cos(alpha) + 0.12 * np.sin(alpha)) * adj
        elif drag_method == "None":
            delta_cd = 0.0
        else:
            raise Exception(
                "The keyword argument darg_method (3d correction for drag) can only be Eggers or None.")
    ...
    def extrapolate(self, cdmax, AR=None, cdmin=0.001, nalpha=15):
        ...
        cl3 = (alpha3 - np.pi) / alpha_high * cl_high * \
        ...
        cl7 = (alpha7 + np.pi) / alpha_high * \
            cl_high * cl_adj  # linear variation
    ...
    def __CMCoeff(self, cl_high, cd_high, cm_high):
        ...
        XM = (-cm_high + cm0) / (cl_high *
                                 np.cos(alpha_high) + cd_high * np.sin(alpha_high))
    ...
    def __getCM(self, i, cmCoef, alpha, cl_ext, cd_ext, alpha_low_deg, alpha_high_deg):
        ...
        if alpha[i] > -165 and alpha[i] < 165:
            if abs(alpha[i]) < 0.01:
                ...
                if alpha[i] > 0:
                    x = cmCoef * \
                        np.tan(np.radians(alpha[i]) - np.pi / 2) + 0.25
                    cm_new = self.cm0 - x * (
                    ...
                    cm_new = -(
                        self.cm0
                        - x * (-cl_ext[i] * np.cos(-np.radians(alpha[i])
                                                   ) + cd_ext[i] * np.sin(-np.radians(alpha[i])))
                    )
        else:
            if alpha[i] == 165:
                ...
                print(
                    "Angle encountered for which there is no CM table value " "(near +/-180 deg). Program will stop.")
        return cm_new
    ...
    def unsteadyParams(self, window_offset=None, nMin=720):
        ...
        if self._radians:
            ...
            cn = cl * np.cos(alpha * np.pi / 180) + cd * \
        ...
        try:
            ...
            print(
                "[WARN] Polar: Cn unsteady, cannot find zero crossing with up direction, trying down direction")
        ...
        try:
            a_MaxUpp, cn_MaxUpp, a_MaxLow, cn_MaxLow = _find_max_points(
                alpha, cn, alpha0, method="inflections")
        except NoStallDetectedException:
            print(
                '[WARN] Polar: Cn unsteady, cannot find stall based on inflections, using min and max')
            a_MaxUpp, cn_MaxUpp, a_MaxLow, cn_MaxLow = _find_max_points(
                alpha, cn, alpha0, method="minmax")
        ...
        cnSlope_poly, a0cn_poly = _find_slope(
            alpha, cn, window=alpha0cn + dwin, method="leastsquare", x0=alpha0cn)
        ...
        a_TSELow, a_TSEUpp = _find_TSE_region(
            alpha, cn, cnSlope, alpha0cn, deviation=0.05)
        ...
        if len(xInter) == 3:
            ...
            print(
                '[WARN] Polar: Cn unsteady, cn_f does not intersect cn 3 times. Intersections:{}.'.format(xInter))
    ...
    def linear_region(self, delta_alpha0=4, method_linear_fit="max"):
        ...
        alpha_linear_region = np.asarray(_find_TSE_region(
            self.alpha, self.cl, cl_slope, alpha0, deviation=0.05))
    ...
    def cl_max(self, window=None):
        ...
        iwindow = np.where(
            (self.alpha >= window[0]) & (self.alpha <= window[1]))
    ...
    def cl_fully_separated(self, method='max'):
        ...
        if cla == 0:
            ...
            I = np.where(fs < 1)
            cl_fs[I] = (self.cl[I] - cla * (self.alpha[I] - alpha0)
                        * fs[I]) / (1.0 - fs[I])
    ...
    def toAeroDyn(self, iTab=None, filenameOut=None, templateFile=None, Re=1.0, comment=None, unsteadyParams=True):
        ...
        if templateFile is None:
            ...
            templateFile = os.path.join(
                MyDir, '../../data/NREL5MW/5MW_Baseline/Airfoils/Cylinder1.dat')
            cleanComments = True
        ...
        ADpol['Re'+labOffset] = np.round(Re*1E-6, 1)  # in millions

        # Compute unsteady parameters
        if unsteadyParams:
            (alpha0, alpha1, alpha2, cnSlope, cn1,
             cn2, cd0, cm0) = self.unsteadyParams()

            # Setting unsteady parameters
            if np.isnan(alpha0):
                ADpol['alpha0'+labOffset] = 0
            else:
                ADpol['alpha0'+labOffset] = np.around(alpha0, 4)
            # TODO approximate
            ADpol['alpha1'+labOffset] = np.around(alpha1, 4)
            # TODO approximate
            ADpol['alpha2'+labOffset] = np.around(alpha2, 4)
            ADpol['C_nalpha'+labOffset] = np.around(cnSlope, 4)
            ADpol['Cn1'+labOffset] = np.around(cn1, 4)    # TODO verify
            ADpol['Cn2'+labOffset] = np.around(cn2, 4)
            ADpol['Cd0'+labOffset] = np.around(cd0, 4)
            ADpol['Cm0'+labOffset] = np.around(cm0, 4)

        # Setting polar
        PolarTable = np.column_stack((self.alpha, self.cl, self.cd, self.cm))
        ADpol['NumAlf'+labOffset] = self.cl.shape[0]
        ADpol['AFCoeff'+labOffset] = np.around(PolarTable, 5)
...
def thicknessinterp_from_one_set(thickness, polarList, polarThickness):
    ...
    for it, t in enumerate(thickness):
        ...
        if ihigh == ilow:
            ...
            print("[WARN] Using nearest polar for section {},   t={} , t_near={}".format(
                it, t, polarThickness[ihigh]))
        else:
            if (polarThickness[ilow] > t) or (polarThickness[ihigh] < t):
                raise Exception("Implementation Error")
            weight = (t - polarThickness[ilow]) / \
                (polarThickness[ihigh] - polarThickness[ilow])
...
def _find_alpha0(alpha, coeff, window, direction='up'):
    ...
    alpha = alpha[iwindow]
    coeff = coeff[iwindow]
    alpha_zc, i_zc, s_zc = _zero_crossings(
        x=alpha, y=coeff, direction=direction)

    if len(alpha_zc) > 1:
        print('WARN: Cannot find alpha0, {} zero crossings of Coeff in the range of alpha values: [{} {}] '.format(
...
def _find_max_points(alpha, coeff, alpha0, method="inflections"):
    ...
    if method == "inflections":
        dC = np.diff(coeff)
        IHatInflections = np.where(np.logical_and.reduce(
            (dC[1:] < 0, dC[0:-1] > 0, alpha[1:-1] > alpha0)))[0]
        IVeeInflections = np.where(np.logical_and.reduce(
            (dC[1:] > 0, dC[0:-1] < 0, alpha[1:-1] < alpha0)))[0]
        if len(IHatInflections) <= 0:
            raise NoStallDetectedException(
                "Not able to detect upper stall point of curve")
        if len(IVeeInflections) <= 0:
            raise NoStallDetectedException(
                "Not able to detect lower stall point of curve")
...
def fn_fullsep(alpha, dclda, alpha0, alpha_sl_neg, alpha_sl_pos, valpha, vCl):
    ...
    cl_linear = cl_lin(alpha, dclda, alpha0, alpha_sl_neg,
                       alpha_sl_pos, valpha, vCl)
...
def cl_fullsep(alpha, dclda, alpha0, alpha_sl_neg, alpha_sl_pos, valpha, vCl, alpha_fs_l, alpha_fs_u):
    ...
    if alpha > alpha_sl_neg and alpha < alpha_sl_pos:
        ...
        fp = f_point(alpha, dclda, alpha0, alpha_sl_neg,
                     alpha_sl_pos, valpha, vCl, alpha_fs_l, alpha_fs_u)
...
def polar_params(alpha, cl, cd, cm):
    ...
    if (y1 < zero_offset)and(y2 < zero_offset):
        ...
        while y1*y2 > 0. and x2+delta < np.pi:
            x1 = x2
            x2 = x1+np.pi/180
            y1 = fn_fullsep(x1, dclda, alpha0, alpha_sl_neg,
                            alpha_sl_pos, alpha, cl)
            y2 = fn_fullsep(x2, dclda, alpha0, alpha_sl_neg,
                            alpha_sl_pos, alpha, cl)
        if y1*y2 < 0:
        ...
        for i, al in enumerate(alpha):
            Cl_fully_sep[i] = cl_fullsep(
                al, dclda, alpha0, alpha_sl_neg, alpha_sl_pos, alpha, cl, alpha_fs_l, alpha_fs_u)
            fs[i] = f_point(al, dclda, alpha0, alpha_sl_neg,
                            alpha_sl_pos, alpha, cl, alpha_fs_l, alpha_fs_u)
            Cl_linear[i] = cl_lin(
                al, dclda, alpha0, alpha_sl_neg, alpha_sl_pos, alpha, cl)
...
def cl_linear_slope(alpha, cl, window=None, method="max", nInterp=721, inputInRadians=False, radians=False):
    ...
    def myret(sl, a0):
        ...
        if radians:
            if inputInRadians:
                ...
                # NOTE: slope needs rad2deg, alpha needs deg2rad
    ...
    if method in ["max", "leastsquare"]:
        slope, off = _find_slope(alpha, cl, xi=alpha0,
                                 window=window, method=method)

    elif method == "leastsquare_constraint":
        slope, off = _find_slope(alpha, cl, x0=alpha0,
                                 window=window, method="leastsquare")

    elif method == "optim":
        # Selecting range of values within window
        idx = np.where((alpha >= window[0]) & (
            alpha <= window[1]) & ~np.isnan(cl))[0]
        ...
        slope, off = _find_slope(alpha, cl, x0=alpha0,
                                 window=None, method="optim")

    else:
        raise Exception(
            "Method unknown for lift slope determination: {}".format(method))

    # --- Safety checks
    if len(cl) > 10:
        # Looking at slope around alpha 0 to see if we are too far off
        slope_FD, off_FD = _find_slope(
            alpha, cl, xi=alpha0, window=window, method="finitediff_1c")
        if abs(slope - slope_FD) / slope_FD * 100 > 50:
            ...
            print('[WARN] More than 20% error between estimated slope ({:.4f}) and the slope around alpha0 ({:.4f}). The window for the slope search ([{} {}]) is likely wrong.'.format(
...
def _find_slope(x, y, xi=None, x0=None, window=None, method="max", opts=None, nInterp=721):
    ...
    if window is not None:
        ...
        if nInterp is not None:
            # using 0.5deg resolution at least
            x_ = np.linspace(x[0], x[-1], max(nInterp, len(x)))
            y_ = np.interp(x_, x, y)
    ...
    if len(y) <= 1:
        raise Exception(
            'Cannot find slope, two points needed ({} after window selection)'.format(len(y)))
    ...
    if method == "max":
        if xi is not None:
            ...
            raise Exception(
                "For now xi needs to be set to find a slope with the max method")
        ...
        if xi is not None:
            ...
            raise Exception(
                "For now xi needs to be set to find a slope with the finite diff method")

    elif method == "leastsquare":
        if x0 is not None:
            try:
                a = np.linalg.lstsq((x - x0).reshape((-1, 1)),
                                    y.reshape((-1, 1)), rcond=None)[0][0][0]
            except:
                a = np.linalg.lstsq((x - x0).reshape((-1, 1)),
                                    y.reshape((-1, 1)))[0][0][0]
        else:
...
def _find_linear_region(x, y, nMin, x0=None):
    ...
    for iStart in range(n):
        for j in range(iStart, n):
            ...
            if x0 is not None:
                sl = np.linalg.lstsq(
                    x[iStart:iEnd], y[iStart:iEnd], rcond=None)[0][0]
                slp[iStart, j] = sl
                off[iStart, j] = x0
...
def _intersections(x1, y1, x2, y2, plot=False, minDist=1e-6, verbose=False):
    ...
    if minDist is not None:
        ...
        for p in zip(x[1:], y[1:]):
            distances = np.array(
                [np.sqrt((p[0]-pk[0])**2 + (p[1]-pk[1])**2) for pk in pointKept])
...
def smooth_heaviside(x, k=1, rng=(-np.inf, np.inf), method="exp"):
    ...
    if method == "exp":
        if np.abs(mn) == np.inf and np.abs(mx) == np.inf:
            ...
            n = 4.0
            # Compact support
            # transform compact support into ]-1,1[
            s = 2.0 / (mx - mn) * (x - (mn + mx) / 2.0)
            # then transform   ]-1,1[  into ]-inf,inf[
            x = -n * s / (s ** 2 - 1.0)
            x[k * x > 100] = 100.0 / k
            x[k * x < -100] = -100.0 / k
