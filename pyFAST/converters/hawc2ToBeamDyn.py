...
def write_beamdyn_sections(filename, span, lK, lM, Mu=None, Label=''):
    ...
    def mat_tostring(M, fmt='.5e'):
        return '\n'.join(['   '+' '.join(['{:.16E}'.format(m) for m in M[i, :]]) for i in range(np.size(M, 1))])
    ...
    with open(filename, 'w') as f:
        ...
        f.write(
            '---------------------- BLADE PARAMETERS --------------------------------------\n')
        f.write(
            '{:5d}  station_total    - Number of blade input stations (-)\n'.format(len(span)))
        f.write(
            '{:5d}  damp_type        - Damping type (switch): 0: no damping; 1: viscous damping\n'.format(damp_type))
        f.write(
            '---------------------- DAMPING COEFFICIENT------------------------------------\n')
...
def htcToBeamDyn(HTCFile, bodyname, BDBldFileOut, BDMainFileOut=None, BDMainTemplate=None, Mu=1.0e-03,
    ...
    H2MeanLine = H2MeanLine[['x_[m]', 'y_[m]',
                             'z_[m]', 'twist_[deg]']]  # Changing order
...
def hawc2ToBeamDyn(H2MeanLine, H2Structure, BDBldFileOut, BDMainFileOut=None, BDMainTemplate=None, Mu=1.0e-03,
    ...
    if zref is None:
        ...
        if z_old[0] != zref[0]:
            raise Exception(
                '`zref` start value should be {} to match input'.format(z_old[0]))
        if z_old[-1] != zref[-1]:
            raise Exception(
                '`zref` end value should be {} to match input'.format(z_old[-1]))
    ...
    # initial_twist [deg] Hawc2 angle is positive around z, unlike the "twist"
    twist = - c2def['twist_[deg]'].values

    # --- Compute r_ref, curvilinear position of keypoints (for st file)
    dr = np.sqrt((x_O_h2[1:]-x_O_h2[0:-1])**2 + (y_O_h2[1:] -
                 y_O_h2[0:-1])**2 + (z_O_h2[1:]-z_O_h2[0:-1])**2)
    ...
    if ref_axis == 'c2def':
        ...
        y_O = 0*y_O_h2   # kp_yr
        # kp_xr NOTE: we fit x_O (where nRoot was already inforced
        x_O = mypolyfit(z_O_h2, x_O, poly_exp)
    ...
    # --- Compute r_ref, curvilinear position of keypoints along BD axis (for st file)
    dr_BD = np.sqrt((x_O[1:]-x_O[0:-1])**2 +
                    (y_O[1:]-y_O[0:-1])**2 + (z_O[1:]-z_O[0:-1])**2)
    r_ref_BD = np.concatenate(([0], np.cumsum(dr_BD)))
    ...
    # For the equations below to be generic we force the column names
    hwc_in.columns = ['r_[m]', 'm_[kg/m]', 'x_cg_[m]', 'y_cg_[m]', 'ri_x_[m]', 'ri_y_[m]', 'x_sh_[m]', 'y_sh_[m]', 'E_[N/m^2]',
                      'G_[N/m^2]', 'I_x_[m^4]', 'I_y_[m^4]', 'I_p_[m^4]', 'k_x_[-]', 'k_y_[-]', 'A_[m^2]', 'pitch_[deg]', 'x_e_[m]', 'y_e_[m]']
    # --- Interpolating to match c2def positions
    hwc = hwc_in
    hwc['r_[m]'] = r_ref_BD
    # note: interpolation is not necessary as all structural data is at same z position
    #hwc = pd.DataFrame(columns=hwc_in.columns)
    #r_old = hwc_in['r_[m]'].values
    # for c in hwc.columns:
    #    hwc[c] = np.interp(r_ref_BD/r_ref_BD[-1], r_old/r_old[-1], hwc_in[c])
    # if r_old[-1]<r_ref[-1]:
    ...
    if any(np.abs(hwc['r_[m]'].values-r_ref_BD) > 1e-9):
        raise Exception(
            'Interpolation failed, radial position mismatch. Debug me.')
    ...
    for i, row in hwc.iterrows():
        ...
        # NOTE: hawc2 and our convention here for angles are positive around z (whereas beamdyn take them negative around z)
        ...
        K = KK(EA, EIxp, EIyp, GKt, GA, kxs, kys, x_C, y_C,
               theta_p, x_S, y_S, theta_s)  # Note theta_p/s in rad
...
#         fig=plt.figure()
        fig, axes = plt.subplots(
            4, 2, sharex=True, figsize=(12.4, 09.))  # (6.4,4.8)
        fig.subplots_adjust(left=0.07, right=0.99, top=0.98,
                            bottom=0.07, hspace=0.25, wspace=0.15)
...
#         ax=axes[0,0]
        ...
        axes[0, 0].text(0.5, 1.01, 'Mean line x', horizontalalignment='center',
                        verticalalignment='bottom', transform=axes[0, 0].transAxes)
        axes[0, 1].text(0.5, 1.01, 'Mean line y', horizontalalignment='center',
                        verticalalignment='bottom', transform=axes[0, 1].transAxes)
        axes[0, 0].plot(z_O, x_O, '-', label='BD smooth)')
        axes[0, 0].plot(z_O, x_O_h2, '--', label='H2 c2def', ms=3, color='k')
        axes[0, 0].plot(z_O, x_off_g, ':',
                        label=r'"$\Delta$" to c2def', color=colrs[6])
        ...
        if 'Relative_thickness_[%]' and 'Chord_[m]' in c2def.columns.values:
            ...
            axes[0, 0].plot(z_O, x_O_h2+c/2*np.sin(twist *
                            np.pi/180), '-', color=[0.5, 0.5, 0.5])
            axes[0, 0].plot(z_O, x_O_h2-c/2*np.sin(twist *
                            np.pi/180), '-', color=[0.5, 0.5, 0.5])
            axes[0, 1].plot(z_O, y_O_h2+c/2*np.cos(twist *
                            np.pi/180), '-', color=[0.5, 0.5, 0.5])
            axes[0, 1].plot(z_O, y_O_h2-c/2*np.cos(twist *
                            np.pi/180), '-', color=[0.5, 0.5, 0.5])
...
#         axes[0,1].set_xlabel('z [m]')
        ...
        vx_G = np.asarray(vx_G)
        vy_G = np.asarray(vy_G)
        vx_S = np.asarray(vx_S)
        vy_S = np.asarray(vy_S)
        vx_C = np.asarray(vx_C)
        vy_C = np.asarray(vy_C)
#         fig,axes = plt.subplots(2, 1, sharex=True, figsize=(6.4,4.8)) # (6.4,4.8)
#         fig.subplots_adjust(left=0.12, right=0.95, top=0.95, bottom=0.11, hspace=0.20, wspace=0.20)
        axes[1, 0].text(0.5, 1.01, 'Abs. position, x', horizontalalignment='center',
                        verticalalignment='bottom', transform=axes[1, 0].transAxes)
        axes[1, 1].text(0.5, 1.01, 'Abs. position, y', horizontalalignment='center',
                        verticalalignment='bottom', transform=axes[1, 1].transAxes)
        axes[1, 0].plot(z_O, x_O, '-', label='BD meanline')
        axes[1, 0].plot(z_O, x_O + vx_G, 'd', ms=6, color=None,
                        markeredgecolor=colrs[1], markerfacecolor="None", label='G (COG)')
        axes[1, 0].plot(z_O, x_O + vx_S, 's', ms=6, color=None,
                        markeredgecolor=colrs[2], markerfacecolor="None", label='S (shear center)')
        axes[1, 0].plot(z_O, x_O + vx_C, 'o', ms=6, color=None, markeredgecolor=colrs[3],
                        markerfacecolor="None", label='C (elastic center)')
        axes[1, 0].plot(hwc['r_[m]'].values, x_O_h2 + hwc['y_cg_[m]'].values,
                        'd', ms=1, color=colrs[1], label='HAWC2')
        axes[1, 0].plot(hwc['r_[m]'].values, x_O_h2 +
                        hwc['y_sh_[m]'].values, 's', ms=1, color=colrs[2])
        axes[1, 0].plot(hwc['r_[m]'].values, x_O_h2 +
                        hwc['y_e_[m]'].values, 'o', ms=1, color=colrs[3])
        axes[1, 1].plot(z_O, y_O, '-', label='BD y (smooth)')
        axes[1, 1].plot(z_O, y_O + vy_G, 'd', ms=6, color=None,
                        markeredgecolor=colrs[1], markerfacecolor="None", label='G (COG)')
        axes[1, 1].plot(z_O, y_O + vy_S, 's', ms=6, color=None,
                        markeredgecolor=colrs[2], markerfacecolor="None", label='S (shear center)')
        axes[1, 1].plot(z_O, y_O + vy_C, 'o', ms=6, color=None, markeredgecolor=colrs[3],
                        markerfacecolor="None", label='C (elastic center)')
        axes[1, 1].plot(hwc['r_[m]'].values, y_O_h2 -
                        hwc['x_cg_[m]'].values, 'd', ms=1, color=colrs[1])
        axes[1, 1].plot(hwc['r_[m]'].values, y_O_h2 -
                        hwc['x_sh_[m]'].values, 's', ms=1, color=colrs[2])
        axes[1, 1].plot(hwc['r_[m]'].values, y_O_h2 -
                        hwc['x_e_[m]'].values, 'o', ms=1, color=colrs[3])
...
#         axes[1,1].set_xlabel('z [m]')
        ...
        if 'Relative_thickness_[%]' and 'Chord_[m]' in c2def.columns.values:
            ...
            axes[1, 0].plot(z_O, x_O_h2+c/2*np.sin(twist *
                            np.pi/180), '-', color=[0.5, 0.5, 0.5])
            axes[1, 0].plot(z_O, x_O_h2-c/2*np.sin(twist *
                            np.pi/180), '-', color=[0.5, 0.5, 0.5])
            axes[1, 1].plot(z_O, y_O_h2+c/2*np.cos(twist *
                            np.pi/180), '-', color=[0.5, 0.5, 0.5])
            axes[1, 1].plot(z_O, y_O_h2-c/2*np.cos(twist *
                            np.pi/180), '-', color=[0.5, 0.5, 0.5])
#
        ...
        axes[2, 0].text(0.5, 1.01, r'Pos. wrt. meanline, x', horizontalalignment='center',
                        verticalalignment='bottom', transform=axes[2, 0].transAxes)
        axes[2, 1].text(0.5, 1.01, r'Pos. wrt. meanline, y', horizontalalignment='center',
                        verticalalignment='bottom', transform=axes[2, 1].transAxes)
        axes[2, 0].plot(z_O, x_O-x_O, '-', label='BD meanline')
        axes[2, 0].plot(z_O, vx_G, 'd', ms=6, color=None,
                        markeredgecolor=colrs[1], markerfacecolor="None", label='G (COG)')
        axes[2, 0].plot(z_O, vx_S, 's', ms=6, color=None, markeredgecolor=colrs[2],
                        markerfacecolor="None", label='S (shear center)')
        axes[2, 0].plot(z_O, vx_C, 'o', ms=6, color=None, markeredgecolor=colrs[3],
                        markerfacecolor="None", label='C (elastic center)')
        axes[2, 0].plot(z_O, x_off_g, ':',
                        label=r'"$\Delta$" to c2def', color=colrs[6])
...
#         axes[1,0].plot(hwc['r_[m]'], x_O_h2 + hwc['y_e_[m]'] , 'd' , ms=1, color=colrs[3] )
        ...
        axes[2, 1].plot(z_O, vy_G, 'd', ms=6, color=None,
                        markeredgecolor=colrs[1], markerfacecolor="None", label='G (COG)')
        axes[2, 1].plot(z_O, vy_S, 's', ms=6, color=None, markeredgecolor=colrs[2],
                        markerfacecolor="None", label='S (shear center)')
        axes[2, 1].plot(z_O, vy_C, 'o', ms=6, color=None, markeredgecolor=colrs[3],
                        markerfacecolor="None", label='C (elastic center)')
        axes[2, 1].plot(z_O, y_off_g, ':',
                        label=r'"$\Delta$" to c2def', color=colrs[6])
...
#         ax=fig.add_subplot(111)
        ax = axes[3, 0]
        ax.text(0.5, 1.01, 'Stiffnesses', horizontalalignment='center',
                verticalalignment='bottom', transform=ax.transAxes)
        ax.plot(z_O, EdgStiff, '-',
                color=colrs[0], label='Edge Stiffness (K_44)')
        ax.plot(z_O, EIxp.values, '--',
                color=colrs[0], label='EIx "edge" at elastic center')
        ax.plot(z_O, FlpStiff, '-',
                color=colrs[1], label='Flap Stiffness (K_55)')
        ax.plot(z_O, EIyp.values, '--',
                color=colrs[1], label='EIy "flap" at elastic center')
        ax.set_xlabel('z [m]')
...
if __name__ == '__main__':
    ...
    # ---  Hawc2 to BeamDyn
    # csv file with c2def columns: ['x_[m]','y_[m]','z_[m]','twist_[deg]']
    H2MeanLine = '../../data/Hawc2/Blade_Planform_Hawc2.csv'
    # csv file with columns ['r_[m]','m_[kg/m]','x_cg_[m]','y_cg_[m]','ri_x_[m]',... ,'x_e_[m]','y_e_[m]']
    H2Structure = '../../data/Hawc2/Blade_Structural_Hawc2.csv'
    # template file to write main BD file
    BDMainTemplate = '../../data/Hawc2/BD.dat'

    BDOut = 'BD_Smooth.dat'
    BDBldOut = 'BD_Blade_Smooth.dat'
    Mu = [0.001]*6  # damping
    hawc2ToBeamDyn(H2MeanLine, H2Structure, BDBldOut, BDOut,
                   BDMainTemplate, Mu=Mu, poly_exp=[2, 3, 4, 5], bPlot=False)
