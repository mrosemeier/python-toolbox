...
def htcToElastoDyn(HTCFile, outDir='./', prefix='', suffix='', bladeBodyName='blade1', towerBodyName='tower', rBlade=None, hTower=None):
    ...
    H2BMeanLine = H2BMeanLine[['x_[m]', 'y_[m]',
                               'z_[m]', 'twist_[deg]']]  # Changing order
    if rBlade is None:
        # NOTE: this should be a curvilinear length, but ElastoDyn is straight, so...
        rB_new = np.abs(H2BMeanLine['z_[m]'].values)
    else:
    ...
    H2TMeanLine = H2TMeanLine[['x_[m]', 'y_[m]',
                               'z_[m]', 'twist_[deg]']]  # Changing order
    if hTower is None:
    ...
    # M[:,1] = PitchAxis          # PitchAxis # PitchAxis in the ElastoDyn blade input file is now used only in conjunction with AeroDyn v14. The specification of aerodynamic center in AeroDyn v15 replaces the need for PitchAxis.
    M[:, 2] = - H2BMeanLine['twist_[deg]']  # StrcTwst [deg]
    M[:, 3] = H2BStructure['m_[kg/m]']   # BMassDen [kg/m]
    # FlpStff [Nm^2] TODO, with twist, make sure that's correct..
    M[:, 4] = H2BStructure['E_[N/m^2]'] * H2BStructure['I_x_[m^4]']
    M[:, 5] = H2BStructure['E_[N/m^2]'] * \
        H2BStructure['I_y_[m^4]']  # EdgStff [Nm^2]
    bld['BldProp'] = M
    # TODO find mode shapes
    bldFileName = os.path.join(
        outDir, '{}ED_blade{}.dat'.format(prefix, suffix))
    ...
    # TwFAStif [Nm^2] TODO TODO TODO VERIFY xy
    M[:, 2] = H2BStructure['E_[N/m^2]'] * H2BStructure['I_x_[m^4]']
    M[:, 3] = H2BStructure['E_[N/m^2]'] * \
        H2BStructure['I_y_[m^4]']  # TwSSStif [Nm^2]
    twr['TowProp'] = M
    # TODO find mode shapes
    twrFileName = os.path.join(
        outDir, '{}ED_tower{}.dat'.format(prefix, suffix))
