...
def multiInterp(x, xp, fp, extrap='bounded'):
    ...
    x = np.asarray(x)
    xp = np.asarray(xp)
    assert fp.shape[1] == len(
        xp), 'Second dimension of fp should have the same length as xp'
...
def interpH2Polar(ae, pc, radius, alpha=None, ae_set_nr=1):
    ...
    """
    # r_ae = zC2  # interpolate w.r.t. pitch axis
    r_ae = ae.ae_sets[ae_set_nr][:, 0]
    r_min = np.min(r_ae)
    r_max = np.max(r_ae)
    if radius < r_min or radius > r_max:
        raise Exception(
            'Radius ({}) needs to be between {} and {}'.format(radius, r_min, r_max))

    thickness = ae.thickness(radius, ae_set_nr)
    pc_set_nr = ae.pc_set_nr(radius, ae_set_nr)
    thicknesses, profiles = pc.pc_sets[pc_set_nr]

    # check if thickness is already present
    if thickness.round(6) in thicknesses.round(6):
        index = np.where(thicknesses.round(6) == thickness.round(6))[0][0]
        M = profiles[index]
        th0 = th1 = thickness
    else:
        # searchsorted assumes ascending order
        index = np.searchsorted(-thicknesses.round(6), -thickness.round(6))
        if index == 0:
            index = 1
        # elif index == len(thicknesses):
        #    index = len(thicknesses)-1
...
def AE_PC_C2_toAD(ae_filename, pc_filename, blade_c2def, r_AD=None, ADbldFilename_out=None, polarFilebase_out='Polar_', ae_set_nr=1, correction3D=True, tsr=9):
    ...
    # in ae file: r = distance from main_body node 1 along z-coordinate [m] (curved length)
    radius_H2 = ae_set[:, 0]

    # --- Default radius if user does not provided one
    if r_AD is None:
        r_AD = zC2  # AeroDyn requires position along pitch axis (not curved)
        #r_AD = radius_H2

    # --- Aerodynamic spanwise data
    # Interpolating AE data to user requested positions
    chord_H2 = np.interp(r_AD, zC2, ae_set[:, 1])
    trel_H2 = np.interp(r_AD, zC2, ae_set[:, 2])
    pcset_H2 = np.interp(r_AD, zC2, ae_set[:, 3])
    ...
    aeroNodes[:, 0] = r_AD
    aeroNodes[:, 1] = yAC_H2  # BlCrvAC # [solved] TODO not c2def but AC
    # BlSwpAC (positive along yOF, negative xH2) # [solved] TODO not c2def but AC
    ...
    aeroNodes[:, 6] = (np.arange(len(r_AD))+1).astype(int)  # BlAFID
    # Write to disk if needed
    if ADbldFilename_out is not None:
        Bld = ADBladeFile()
        Bld.comment = 'Generated using HAWC2 inputs: AE:{} PC:{}'.format(
            ae_base, pc_base)
        Bld['BldAeroNodes'] = aeroNodes
        Bld.write(ADbldFilename_out)
    aeroNodes = pd.DataFrame(data=aeroNodes, columns=[
                             'BlSpn_[m]', 'BlCrvAC_[m]', 'BlSwpAC_[m]', 'BlCrvAng_[deg]', 'BlTwist_[deg]', 'BlChord_[m]', 'BlAFID_[-]'])
    ...
    r_max = np.max(radius_H2)
    # do interpolation along H2 radius (arc length)
    for ir, (r, c) in enumerate(zip(radius_H2, chord_H2)):
        M, t, pc_set, thicknesses = interpH2Polar(
            ae.data, pc.data, r, alpha=vAlpha, ae_set_nr=ae_set_nr)
        comment = 'Thickness: {} - pc_set:{} - thicknesses:{}\nGenerated using HAWC2 inputs: AE:{} PC:{}'.format(
            t, pc_set, thicknesses, ae_base, pc_base)
        Re = 5.0  # TODO
        ...
        if writePolars:
            ...
            P.toAeroDyn(filenameOut=polarFilename_out,
                        templateFile=pol, Re=Re, comment=comment)
...
def hawc2toAD(htc, r_AD=None, ADbldFilename_out=None, polarFilebase_out='Polar_', bladeBodyName='blade1', correction3D=True, tsr=9):
    ...
    if 'aero' not in htc.data.keys():
        raise Exception(
            'Aero section not found in htc file: '.format(htcFilename))
    aero = htc.data['aero']
